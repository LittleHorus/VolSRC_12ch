...
uint16_t adc_buf[24];
uint16_t adc_for_disp[24], adc_prev[24];
...
void adc_offset_cal_and_flash_write(){
  for(uint16_t istep = 0;istep < 1024;istep++){

    dac_spi_custom(0, (0x310000 |  (istep<<6))); 
    delay_nop(50000);
    
    adc_offset_cal[istep] = adc_data_via_spi(0, 0);
    delay_nop(500);
  }
  dac_spi_custom(0, (0x310000 |  (0<<6))); 
        ...
        for(uint16_t a=0; a<1024; a++){
          FLASH_ProgramHalfWord(ADRESS_FLASH_OFFSET + 2*a, adc_offset_cal[a]);
        }
        FLASH_Lock();    
  
}
void adc_read_offset(){
   for(uint16_t a=0; a<1024;a++){
    //adc_offset_cal[a] = flash_read(ADRESS_FLASH_OFFSET + a*2);
    //if(adc_offset_cal[a] == 0xff) adc_offset_cal[a] = 0x00;//if flash empty
    adc_offset_cal[a] = ((uint16_t) a/4); 
...
void read_voltage_values(){
  ...
  for(uint8_t a=0; a<12;a++){
    ...
    if(temp_voltage_value[a] == 0xff) temp_voltage_value[a] = 0x00;//if flash empty
  }
...
void ft812_12ch_disp(){
    ...
    if(saved_timer != 0) FT8_cmd_text(160,23, 22, 0, "saved");
    ...
    FT8_cmd_text(advr_x_offset,225, 18, 0, "sell garage");
    advr_x_offset++;
    if(advr_x_offset >= 320) advr_x_offset = 0;
...
void conv_12ch_data(uint16_t *adc_data){
  float temp_data[24];
  //-adc_offset_cal[temp_voltage_value[4*adc_i]]
  adc_uA_num[0] = temp_voltage_value[0];
  adc_uA_num[1] = temp_voltage_value[1];
  adc_uA_num[2] = temp_voltage_value[2];
  adc_uA_num[3] = temp_voltage_value[3];
  adc_uA_num[4] = temp_voltage_value[4];
  adc_uA_num[5] = temp_voltage_value[10];
  
  for(uint8_t adc_i = 0; adc_i < 6; adc_i++){
    temp_data[4*adc_i] = 165*((float)(adc_data[4*adc_i] - adc_offset_cal[adc_uA_num[adc_i]])/4095);//uA // V/200ohm /100  /20000 = A | *10 = uA
    temp_data_monitor[adc_i] = (adc_data[4*adc_i] - adc_offset_cal[adc_uA_num[adc_i]]);
    //else temp_data[4*adc_i] = 165*((float)adc_data[4*adc_i]/4095);//uA // V/200ohm /100  /20000 = A | *10 = uA
...
int main(void)
{
  ...
  {   
    ...
    if(butn2_cnt == 0) step_size = 1;
    if(butn2_cnt == 1) step_size = 5;
    if(butn2_cnt == 2) step_size = 10;
    if(butn2_cnt == 3) butn2_cnt = 0;
   
    if(average_delay == 0){
      if(av_count < 20)av_count++;     
      for(uint8_t adc_ch = 0; adc_ch < 24; adc_ch++){        
        ...
        if(((uint16_t) adc_av_buf[adc_ch]) >= adc_prev[adc_ch]){
          if((((uint16_t) adc_av_buf[adc_ch]) - adc_prev[adc_ch]) > 4) {
            adc_for_disp[adc_ch] = (uint16_t) adc_av_buf[adc_ch]; 
            adc_prev[adc_ch] = (uint16_t) adc_av_buf[adc_ch];
          }
          else adc_for_disp[adc_ch] = adc_prev[adc_ch];       
        }
        else{
          if(((adc_prev[adc_ch] - (uint16_t) adc_av_buf[adc_ch])) > 4) {
            adc_for_disp[adc_ch] = (uint16_t) adc_av_buf[adc_ch]; 
            adc_prev[adc_ch] = (uint16_t) adc_av_buf[adc_ch];
          }
          else adc_for_disp[adc_ch] = adc_prev[adc_ch];            
      ...
      if(spi_queue != 0){
            ...
            if(spi_cs_buf[spi_tail] == 5) dac_spi_custom(0, (0x320000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 6) dac_spi_custom(0, (0x340000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 1) dac_spi_custom(0, (0x380000 |  (spi_data_buf[spi_tail]<<6)));
            
            if(spi_cs_buf[spi_tail] == 2) dac_spi_custom(1, (0x310000 |  (spi_data_buf[spi_tail]<<6))); 
            if(spi_cs_buf[spi_tail] == 7) dac_spi_custom(1, (0x320000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 8) dac_spi_custom(1, (0x340000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 3) dac_spi_custom(1, (0x380000 |  (spi_data_buf[spi_tail]<<6)));

            if(spi_cs_buf[spi_tail] == 4) dac_spi_custom(2, (0x310000 |  (spi_data_buf[spi_tail]<<6))); 
            if(spi_cs_buf[spi_tail] == 9) dac_spi_custom(2, (0x320000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 11) dac_spi_custom(2, (0x340000 |  (spi_data_buf[spi_tail]<<6)));
            if(spi_cs_buf[spi_tail] == 10) dac_spi_custom(2, (0x380000 |  (spi_data_buf[spi_tail]<<6)));            
...
void TIM7_IRQHandler()
{
 ...
 if(encoder_timer == 1){
  ...
  if(((encoder_10_value & 0x00C0)>>6) == 3) encoder_10_state = 0;
  if(((encoder_10_value & 0x00C0)>>6) == 2) encoder_10_state = 1;
  if(((encoder_10_value & 0x00C0)>>6) == 1) encoder_10_state = 2;
  if(((encoder_10_value & 0x00C0)>>6) == 0) encoder_10_state = 3;
